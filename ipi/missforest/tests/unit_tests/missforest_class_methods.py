...
class MissForestClassMethods(unittest.TestCase):
    def setUp(self):
        ...
        while True:
            ...
            if (
                self.train.isnull().sum().sum() > 0
                and self.test.isnull().sum().sum() > 0
            ):
                break
    ...
    def test_get_missing_rows(self):
        ...
        df = pd.DataFrame(
            {"A": [1, 2, None, 4], "B": [None, 2, 3, 4], "C": [1, 2, 3, None]}
        )
    ...
    def test_get_initials_mean(self):
        ...
        self.assertEqual(
            self.missforest._compute_initial_imputations(df, ["B"]),
            {"A": 2.5, "B": "a"},
        )
    ...
    def test_get_initials_median(self):
        ...
        under the circumstance that argument `initial_guess` of MissForest
        is set to `median`."""
        df = pd.DataFrame({"A": [1, 2, 2, 4], "B": ["a", "a", "b", "c"]})
    ...
    def test_initial_imputation_mean(self):
        ...
        `mean`."""
        df = pd.DataFrame(
            {"A": [1, 2, None, 4], "B": [None, 2, 3, 4], "C": ["a", "a", "b", "c"]}
        )

        self.missforest.initial_guess = "mean"
        initial_imputations = self.missforest._compute_initial_imputations(df, ("C"))
